...
import GamingBg from "../../../assets/images/Gaming-bg.jpg";
...
export default function Gaming() {
    ...
    const fetchAllProducts = async () => {
      try {
        const res = await fetch(
          `${"https://http://swapmeet.liveblog365.com/api"}/get-all-products.php`
        );
  ...
  const dynamicLocations = [
    ...new Set(gaming.map((p) => p.location).filter(Boolean)),
  ];
  ...
  //— Apply filters
  const filtered = gaming.filter((product) => {
    return (
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <div className="relative h-60 md:h-150">
        ...
        />
        <h1 className="absolute top-[50%] left-[16%] text-6xl md:text-5xl md:top-[40%] md:left-[20%] cursor-default text-white font-bold md:text-12xl">
          Gaming
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 p-6">
        ...
        <div className="md:col-span-3 space-y-4 md:-translate-x-20">
          {/* Search Bar */}
          <form className="flex md:block bg-white p-4 ring-1 ring-black/10 shadow-lg rounded-2xl">
            <input
              type="text"
