...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      const res = await fetch(
        `https://swapmeet-backend-706493566423.us-central1.run.app/seller-dashboard.php?sellerId=${user.sellerProfile.sellerID}`
      );
  ...
  useEffect(() => {
    ...
    const fetchUserItems = async () => {
      try {
        const res = await fetch(
          `https://swapmeet-backend-706493566423.us-central1.run.app/get-seller-products.php?sellerId=${user.sellerProfile.sellerID}`
        );
        const data = await res.json();
        if (data.success) {
