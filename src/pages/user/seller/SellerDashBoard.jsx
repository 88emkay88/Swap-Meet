...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      const res = await fetch(
        `https://swapmeet-backend.byethost12.com/api/seller-dashboard.php?sellerId=${user.sellerProfile.sellerID}`
      );
  ...
  useEffect(() => {
    ...
    const fetchUserItems = async () => {
      try {
        const res = await fetch(
          `https://swapmeet-backend.byethost12.com/api/get-seller-products.php?sellerId=${user.sellerProfile.sellerID}`
        );
        const data = await res.json();
        if (data.success) {
