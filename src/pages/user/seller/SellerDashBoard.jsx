...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      const res = await fetch(
        `https://swapmeet-backend.webze.eu.org/api/seller-dashboard.php?sellerId=${user.sellerProfile.sellerID}`
      );
  ...
  useEffect(() => {
    ...
    const fetchUserItems = async () => {
      try {
        const res = await fetch(
          `https://swapmeet-backend.webze.eu.org/api/get-seller-products.php?sellerId=${user.sellerProfile.sellerID}`
        );
        const data = await res.json();
        if (data.success) {
