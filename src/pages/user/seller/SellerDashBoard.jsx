...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      const res = await fetch(
        `swapmeet-backend.byethost12.com/seller-dashboard.php?sellerId=${user.sellerProfile.sellerID}`
      );
  ...
  useEffect(() => {
    ...
    const fetchUserItems = async () => {
      try {
        const res = await fetch(
          `swapmeet-backend.byethost12.com/get-seller-products.php?sellerId=${user.sellerProfile.sellerID}`
        );
        const data = await res.json();
        if (data.success) {
